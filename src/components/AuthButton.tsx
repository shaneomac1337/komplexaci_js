...
export function AuthButton({ variant = 'default' }: AuthButtonProps) {
  ...
  if (variant === 'pill') {
    if (isLoading) {
      return (
        <div className="pill-nav-auth pill-nav-auth-loading" aria-hidden="true">
    ...
    if (!isAuthenticated) {
      return (
        <button
          ...
          className="pill-nav-auth pill-nav-login"
    ...
    return (
      <div className="pill-nav-auth pill-nav-user">
        ...
        <span className="pill-nav-username">{user?.name}</span>
        <button
          onClick={logout}
