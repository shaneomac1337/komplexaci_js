...
export default function LeagueOfLegendsNextJS() {
  ...
  const mapDamageType = (name: string, tags: string[]) => {
    ...
    if (magicChampions.includes(name)) return 'Magic';
    if (physicalChampions.includes(name)) return 'Physical';
    if (tags.includes('Mage')) return 'Magic';
    if (tags.includes('Marksman')) return 'Physical';
    return 'Mixed';
  };

  const mapRoles = (tags: string[], name: string) => {
    // Official position mappings based on provided data
    ...
    if (championRoles[name]) {
      return championRoles[name];
    }
  ...
  const mapRangeType = (name: string, tags: string[]) => {
  ...
  const fetchChampions = async () => {
    try {
      ...
      const response = await fetch(`https://ddragon.leagueoflegends.com/cdn/${version}/data/cs_CZ/champion.json`);
      ...
      const championList = Object.values(data.data).map((champion: any) => ({
        ...
        damage: mapDamageType(champion.name, champion.tags),
        survivability: mapSurvivability(champion.info.defense),
        roles: mapRoles(champion.tags, champion.name),
        rangeType: mapRangeType(champion.name, champion.tags),
  ...
  const fetchChampionDetails = async (championId: string) => {
    try {
      ...
      const response = await fetch(`https://ddragon.leagueoflegends.com/cdn/${currentVersion}/data/cs_CZ/champion/${championId}.json`);
                          ...
                          <div className={styles.championStats}>
                            ...
                            <div className={styles.statsGrid}>
                              ...
                              <div className={styles.statItem}>
                                ...
                                <span className={styles.statValue}>{championDetails.stats.movespeed}</span>
                              </div>
                              <div className={styles.statItem}>
                              ...
                              <div className={styles.statItem}>
                                <span className={styles.statLabel}>Mag. odolnost:</span>
